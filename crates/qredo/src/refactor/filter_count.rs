...
const MESSAGE: &str = "`Enum.count/2` is more efficient than `Enum.filter/2 |> Enum.count/1`.";
const TRIGGER: &str = "count";

/// `EX4030`: `Enum.filter |> Enum.count`.
pub(crate) fn check_prepared(prepared: &crate::batch::Prepared<'_>) -> Vec<Finding> {
    let mut pair = EnumPair::new("filter", "count", MESSAGE, TRIGGER);
    pair.outer_arity = Some(1);
    pair.rhs_empty = true;
    pair.check(prepared.masked())
}
...
mod tests {
    use super::*;
    #[test]
    fn direct_count_is_clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("Enum.count(x, & &1)\n")).is_empty());
    }
    #[test]
    fn reports_filter_count() {
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(
                "x |> Enum.filter(& &1) |> Enum.count()\n"
            ))
            .len(),
            1
        );
    ...
    fn reports_pipe_chain_like_upstream() {
        // EX4030.upstream.violation-pipe: trigger is `count`.
        ...
        assert_eq!((findings[0].line, findings[0].column), (5, Some(13)));
    }
    #[test]
    fn count_with_fun_is_clean() {
        // EX4030.upstream.not-filter-count-2-pipe: `Enum.count/2` stays.
