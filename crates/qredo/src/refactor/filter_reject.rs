...
const MESSAGE: &str = "One `Enum.filter/2` is more efficient than `Enum.filter/2 |> Enum.reject/2`";
const TRIGGER: &str = "|>";

/// `EX4009`: `Enum.filter |> Enum.reject`.
pub(crate) fn check_prepared(prepared: &crate::batch::Prepared<'_>) -> Vec<Finding> {
    EnumPair::new("filter", "reject", MESSAGE, TRIGGER).check(prepared.masked())
}
...
mod tests {
    ...
    #[test]
    fn reports_filter_reject() {
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(
                "x |> Enum.filter(& &1) |> Enum.reject(& &2)\n"
            ))
            .len(),
            1
        );
    ...
    fn reports_pipe_chain_like_upstream() {
        // EX4009.upstream.violation: trigger is `|>`, column points at it.
