...
const SECOND: &str = "Enum.reject";
const MESSAGE: &str = "One `Enum.reject/2` is more efficient than `Enum.reject/2 |> Enum.reject/2`";
...
mod tests {
    ...
    #[test]
    fn clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("Enum.reject(x, & &1)\n")).is_empty());
    }
    #[test]
    fn reports() {
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(
                "x |> Enum.reject(& &1) |> Enum.reject(& &2)\n"
            ))
            .len(),
            1
        );
