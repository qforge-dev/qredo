...
const MESSAGE: &str = "`Enum.map_join/3` is more efficient than `Enum.map/2 |> Enum.join/2`.";
const SECOND: &str = "join";
...
pub(crate) fn check_prepared(prepared: &crate::batch::Prepared<'_>) -> Vec<Finding> {
    ...
    for call in scan.calls(SECOND) {
        ...
        if args.len() == 2 && scan.call_at(args[0].0, args[0].1, "map").is_some() {
            push(&raw_lines, &mut findings, call.line, "Enum.join");
        } else if scan
...
mod tests {
    ...
    #[test]
    fn clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("Enum.join(x)\n")).is_empty());
    }
    #[test]
    fn reports() {
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(
                "x |> Enum.map(& &1) |> Enum.join()\n"
            ))
            .len(),
            1
        );
    ...
    #[test]
    fn pipe_pair_reports_pipe_trigger() {
