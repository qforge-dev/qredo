...
const MESSAGE: &str = "`Enum.into/3` is more efficient than `Enum.map/2 |> Enum.into/2`.";
const SECOND: &str = "into";
...
pub(crate) fn check_prepared(prepared: &crate::batch::Prepared<'_>) -> Vec<Finding> {
    ...
    for call in scan.calls(SECOND) {
        ...
        if args.len() == 2 && scan.call_at(args[0].0, args[0].1, "map").is_some() {
            push(&raw_lines, &mut findings, call.line, "Enum.into");
        } else if scan
    ...
    for pipe in scan.pipes() {
        let Some((enum_pos, into_open)) = scan.right_call(pipe, SECOND) else {
            continue;
        };
        let Some(enum_start) = scan.map_before(pipe) else {
            continue;
        };
        if scan.pipe_before(enum_start) {
            if scan
                .arg_spans(into_open, scan.match_paren(into_open).unwrap_or(into_open))
                .len()
                == 1
            {
...
impl<'a> Scan<'a> {
    ...
    /// Whether another `|>` feeds the call at `enum_start` (pipe chain).
    fn pipe_before(&self, enum_start: usize) -> bool {
        let pos = self.skip_ws_back(enum_start);
        pos >= 2 && &self.bytes[pos - 2..pos] == b"|>" && (pos < 3 || self.bytes[pos - 3] != b'<')
    }
}
...
mod tests {
    ...
    #[test]
    fn clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("Enum.into(x, %{})\n")).is_empty());
    }
    #[test]
    fn reports() {
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(
                "x |> Enum.map(& &1) |> Enum.into(%{})\n"
            ))
            .len(),
            1
        );
