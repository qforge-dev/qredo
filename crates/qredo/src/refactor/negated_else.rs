...
const MESSAGE: &str = "Avoid negated conditions in if-else blocks.";
...
pub(crate) fn check_prepared(prepared: &crate::batch::Prepared<'_>) -> Vec<Finding> {
    ...
    for pos in scan.keywords() {
        let Some(neg) = scan.negation(pos + "if".len()) else {
            continue;
        };
        if scan.has_else_block(pos) {
            let line = scan.line_of(pos);
            findings.push(Finding::with_trigger(
                line + 1,
                raw_lines
                    .get(line)
                    .and_then(|l| trigger_column(l, neg.trigger)),
                MESSAGE,
                neg.trigger.to_owned(),
            ));
        }
    }
...
impl<'a> Scan<'a> {
    ...
    /// `if` call sites, skipping atoms, access, attributes, and defs.
    fn keywords(&self) -> Vec<usize> {
        let mut out = Vec::new();
        let mut idx = 0_usize;
        while idx + "if".len() <= self.bytes.len() {
            if self.at_word(idx, "if") && self.is_call(idx) {
                out.push(idx);
                idx += "if".len();
            } else {
                idx += 1;
    ...
    /// condition, unwrapping every paren layer the way Credo's `__block__`
    /// recursion does.
    fn negation(&self, from: usize) -> Option<Negation<'_>> {
        ...
        loop {
            if stop > start
                && self.bytes[start] == b'('
                && self.match_paren(start) == Some(stop - 1)
            {
        ...
        if self.at_word(start, "not") {
            ...
            if self.gap_is_ws(op, stop) {
                return Some(Negation { trigger: "not" });
            }
        }
        None
    }
    ...
    /// Whether an `else` keyword at `idx` belongs to our `if`.
    fn is_own_else(&self, idx: usize, inline: bool, depth: i64) -> bool {
        if !self.at_word(idx, "else") || !self.plain_prev(idx) {
            return false;
        }
        let colon = self.bytes.get(idx + 4) == Some(&b':');
        (!inline && !colon && depth == 1) || (inline && depth == 0)
    }

    /// Step over `fn`/`end`/brackets; false ends the scan without `else`.
    fn step_plain(&self, idx: &mut usize, depth: &mut i64) -> bool {
        if self.at_word(*idx, "fn") && self.plain_prev(*idx) {
            *depth += 1;
            *idx += 2;
            return true;
    ...
    /// Whether the `if` construct carries an `else` branch: the first
    /// `do`/`do:` opens it, a depth-zero `end` closes it without `else`.
    fn has_else_block(&self, kw_pos: usize) -> bool {
        let mut depth = 0_i64;
        let mut inline = false;
        let mut idx = kw_pos + "if".len();
        ...
        while idx < self.bytes.len() {
            ...
            if self.at_word(idx, "else") && self.plain_prev(idx) {
                if self.is_own_else(idx, inline, depth) {
                    return true;
                }
                idx += 4;
                continue;
            }
            if !self.step_plain(&mut idx, &mut depth) {
                return false;
            }
        }
        false
...
mod tests {
    ...
    #[test]
    fn parenthesized_condition_reports_negation() {
