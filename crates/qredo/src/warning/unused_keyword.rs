...
const CHECKED_MODULE: &str = "Keyword";
const ISSUE_MESSAGE: &str = "There should be no unused return values for `Keyword` functions.";
...
fn decide(parent: tree_sitter::Node, child: tree_sitter::Node, bytes: &[u8]) -> Decision {
    match parent.kind() {
        ...
        // Unary/access/dot parents consume the value upstream (the `.`
        // operator call verifies its function argument, so
        // `Keyword.get(y, :a).()` is used).
        "unary_operator" | "access_call" | "dot" => Decision::Used,
...
mod tests {
    ...
    fn clean() {
        assert!(
            check_prepared(&crate::batch::Prepared::lazy("x = Keyword.get(y, :a)\n")).is_empty()
        );
    }
    #[test]
    fn reports() {
    ...
    fn dot_function_position_is_used() {
        ...
        // function argument, so `Keyword.get(y, :a).()` uses the result
        // even in a discarded tail (after-block, non-tail statement).
        let source =
            "defmodule M do\n  def f(y) do\n    Keyword.get(y, :a).()\n    :ok\n  end\nend\n";
        assert!(check_prepared(&crate::batch::Prepared::lazy(source)).is_empty());
        let after_call = "defmodule M do\n  def f(y) do\n    try do\n      :ok\n    after\n      Keyword.get(y, :a).()\n    end\n  end\nend\n";
        assert!(check_prepared(&crate::batch::Prepared::lazy(after_call)).is_empty());
    }
}
