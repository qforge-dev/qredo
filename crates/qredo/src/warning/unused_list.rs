...
const CHECKED_MODULE: &str = "List";
const ISSUE_MESSAGE: &str = "There should be no unused return values for `List` functions.";
...
mod tests {
    ...
    #[test]
    fn clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("x = List.first(y)\n")).is_empty());
    }
    #[test]
    fn reports() {
    ...
    fn buried_call_reports_with_column() {
        let source = "defmodule CredoSampleModule do\n  defp print_issue(issue) do\n    if issue.column do\n      IO.puts \".\"\n    else\n      [:this_goes_nowhere, List.delete_at(w, \",\")]\n    end\n\n    IO.puts\n  end\nend\n";
