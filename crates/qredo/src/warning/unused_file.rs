...
const CHECKED_MODULE: &str = "File";
const ISSUE_MESSAGE: &str = "There should be no unused return values for `File` functions.";
const RETURNING_FUNS: Option<&[&str]> = Some(&[
    "cwd", "cwd!", "dir?", "exists?", "read", "read!", "regular?", "stat", "stat!",
]);
...
mod tests {
    ...
    #[test]
    fn clean() {
        assert!(check_prepared(&crate::batch::Prepared::lazy("x = File.read(y)\n")).is_empty());
    }
    #[test]
    fn reports() {
