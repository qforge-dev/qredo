...
pub(crate) fn check_prepared(
    ...
    run(prepared, &ignored_funs(params))
}

/// Function names excluded via the `ignore` param (e.g. `["reduce"]`).
fn ignored_funs(params: &BTreeMap<String, String>) -> Vec<String> {
...
const CHECKED_MODULE: &str = "Enum";
const ISSUE_MESSAGE: &str = "There should be no unused return values for `Enum` functions.";
const RETURNING_FUNS: Option<&[&str]> = Some(&[
    "all?",
    "any?",
    "at",
    "chunk",
...
fn run(prepared: &crate::batch::Prepared<'_>, ignored: &[String]) -> Vec<Finding> {
...
mod tests {
    ...
    #[test]
    fn assigned_is_clean() {
        assert!(
            check_prepared(
                &crate::batch::Prepared::lazy("x = Enum.map(y, & &1)\n"),
                &params()
            )
            .is_empty()
        );
    ...
    fn reports_bare() {
        ...
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(source), &params()).len(),
            1
        );
    }
    #[test]
    fn piped_result_is_clean() {
        ...
        assert!(check_prepared(&crate::batch::Prepared::lazy(source), &params()).is_empty());
    }
    #[test]
    fn buried_call_reports_with_column() {
        let source = "defmodule CredoSampleModule do\n  defp print_issue(issue) do\n    if issue.column do\n      IO.puts \".\"\n    else\n      [:this_goes_nowhere, Enum.join(w, \",\")]\n    end\n\n    IO.puts\n  end\nend\n";
        let findings = check_prepared(&crate::batch::Prepared::lazy(source), &params());
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].line, 6);
        assert_eq!(findings[0].column, Some(28));
    }
    #[test]
    fn ignored_functions_are_clean() {
        let source =
            "defmodule M do\n  def f(y) do\n    Enum.reduce(y, 0, & &1)\n\n    y\n  end\nend\n";
        let mut ignored = BTreeMap::new();
        ignored.insert("ignore".to_owned(), "[\"reduce\"]".to_owned());
        assert!(check_prepared(&crate::batch::Prepared::lazy(source), &ignored).is_empty());
        assert_eq!(
            check_prepared(&crate::batch::Prepared::lazy(source), &params()).len(),
            1
        );
